...
function addFrameworkAndSearchPath() {
  ...
  var frameworkPath = path.join(__dirname,'../node_modules/@cross2d/react-native-ushare/ios/UMSocial');
      ...
      if (error) {
        ...
        if (project.hasFile(file.path)) return false;
         ...
         //project.addToFrameworkSearchPaths(file);
          addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/UMSocial"');
          addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/UMSocialSDK"');
          addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/QQ"');
          addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/Sina"');
          addSearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/WeChat"');
          addLibrarySearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/QQ"');
          addLibrarySearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/Sina"');
          addLibrarySearchPaths(project,'"$(SRCROOT)/../node_modules/@cross2d/react-native-ushare/ios/RCTUShare/SocialLibraries/WeChat"');
...
function addLibrarySearchPaths(project, librarySearchPath) {
    const config = project.pbxXCBuildConfigurationSection();
    const INHERITED = '"$(inherited)"';
    Object.keys(config).filter(ref => ref.indexOf('_comment') === -1)
        .forEach( ref => {
            const buildSettings = config[ref].buildSettings;
            const shouldVisitBuildSettings = (
                buildSettings['PRODUCT_NAME'] === package.name);
            if (shouldVisitBuildSettings) {
                ...
                var framworkIndex = _.findIndex(buildSettings['LIBRARY_SEARCH_PATHS'], function(path) { return path == librarySearchPath; });
                if (framworkIndex === -1) {
                    buildSettings['LIBRARY_SEARCH_PATHS'].push(librarySearchPath);
                }
            }
        });
};
...
// addAppID();
// addQueriesSchemes();
addFrameworkAndSearchPath();var fs = require('fs');
var glob = require("glob");
var inquirer = require('inquirer');
var xcode = require('xcode');
var path = require('path');
